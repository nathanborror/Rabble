...
public class IRCClient: IRCClientMessageTarget {
    ...
    public private(set) var userMode = IRCUserMode()
        ...
        _ = bootstrap.channelInitializer { [weak self] channel in
            return channel.pipeline
                ...
                .flatMap { [weak self] _ in
                    ...
                    return channel.pipeline
                        .addHandler(Handler(client: me), name: "de.zeezide.nio.irc.client")
                }
        }
    }
    ...
    public func changeNick(_ nick: IRCNickName) {
        send(.NICK(nick))
    }
    ...
    public func connect() {
        guard eventLoop.inEventLoop else {
            return eventLoop.execute(self.connect)
        }
    ...
    open func close() {
        guard eventLoop.inEventLoop else {
            return eventLoop.execute(close)
        }
        _ = channel?.close(mode: .all)
        clearListCollectors()
    }
...
//    }
    ...
    func handlerCaughtError(_ error: Swift.Error, in context: ChannelHandlerContext) {
    ...
    public func sendMessages<T: Collection>(_ messages: T, promise: EventLoopPromise<Void>?) where T.Element == IRCMessage {
...
extension IRCClient: IRCDispatcher {
    ...
    public func doNotice(recipients: [ IRCMessageRecipient ], message: String) throws {
        delegate?.client(self, notice: message, for: recipients)
    }

    public func doMessage(sender: IRCUserID?, recipients: [IRCMessageRecipient], message: String) throws {
    ...
    public func doMode(nick: IRCNickName, add: IRCUserMode, remove: IRCUserMode) throws {
