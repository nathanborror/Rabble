...
extension FilesProvider {
    ...
    public func cachedFileMetadata(path: String) throws -> File {
        guard let out = files.first(where: { $0.path == path }) else {
            throw Error.cachedMetadataNotFound
        }
        return out
    }

    public func cachedFileObject<T: Decodable>(_ fileID: String) throws -> T {
        let data = try cachedFileData(fileID)
        return try decoder.decode(T.self, from: data)
    }

    public func cachedFilePackage<T: Packagable>(_ fileID: String) throws -> T {
        let file = try cachedFileMetadata(fileID)
        return try T.load(url: .documentsDirectory.appending(path: file.path))
    }
    ...
    public func cacheFilePackage<T: Packagable>(_ package: T, fileID: String) async throws {
        try await ready()
        let file = try cachedFileMetadata(fileID)
        for (url, data) in try package.write(url: .documentsDirectory.appending(path: file.path)) {
            try cache(data: data, to: url)
        }
    }
