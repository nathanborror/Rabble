...
extension IRC {
    ...
    public enum Numeric: Codable, Equatable, Sendable {
        ...
        case RPL_ENDOFNAMES(client: String, channel: String, message: String)
        ...
        case RPL_MOTD(client: String, message: String)
        ...
        case RPL_MOTDSTART(client: String, message: String)
        ...
        case RPL_ENDOFMOTD(client: String, message: String)
        ...
        public init?(_ string: String, params: [String]) {
            ...
            case 366:
                guard params.count >= 3 else { return nil }
                self = .RPL_ENDOFNAMES(client: params[0], channel: params[1], message: params[2])
            case 372:
                guard params.count >= 2 else { return nil }
                self = .RPL_MOTD(client: params[0], message: params[1])
            case 375:
                guard params.count >= 2 else { return nil }
                self = .RPL_MOTDSTART(client: params[0], message: params[1])
            case 376:
                guard params.count >= 2 else { return nil }
                self = .RPL_ENDOFMOTD(client: params[0], message: params[1])
            case 396:
                self = .RPL_HOSTHIDDEN
